...
const {sender_user_email,passwordEmail}=process.env
const sendMail=(to,url,txt)=>{
    const transport =  nodemailder.createTransport({ // config mail server
      service: 'gmail',
      auth: {
        user: sender_user_email,
        pass: passwordEmail,
      }
  })
